...
class llm4rec(nn.Module):
    def __init__(self, device, llm_model="", mean_resizing=True, max_output_txt_len=256,):
        super().__init__()
        self.device = device
        self.llm = llm_model

        if llm_model == 'opt':
            self.llm_model = OPTForCausalLM.from_pretrained("facebook/opt-6.7b",
                                                            torch_dtype=torch.float16, load_in_8bit=True, device_map=self.device)
            self.llm_tokenizer = AutoTokenizer.from_pretrained("facebook/opt-6.7b", use_fast=False)
        elif llm_model == 'llama':
            self.llm_model = LlamaForCausalLM.from_pretrained("meta-llama/Llama-2-7b-hf",
                                                              torch_dtype=torch.float16, load_in_8bit=True, device_map=self.device)
            self.llm_tokenizer = AutoTokenizer.from_pretrained("meta-llama/Llama-2-7b-hf", use_fast=False)
        ...
        self.llm_model.resize_token_embeddings(len(self.llm_tokenizer), mean_resizing=False)
